...
const ChatList = ({ contacts, addNewContact, selectedChatId }) => {
    ...
    <div className="chat-list">
        ...
        {filteredContacts.length === 0 ? (
            <span className="no-contacts">No se encontraron contactos</span>
        ) : (
            filteredContacts.map((contact) => {
            const isActive = Number(contact.id) === Number(selectedChatId);
                ...
                <Link to={`/chat/${contact.id}`} className="chat-item-link">
                    ...
                    <div className="chat-item-bottom">
                        ...
                        className={`chat-item-status ${
                            contact.isConected ? "chat-item-status--online" : ""
                        }`}
                        >
                        {contact.isConected
                            ? "En línea"
